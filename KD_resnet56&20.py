...
def main():
   ...
   # teacher_ckpt = os.path.join(project_dir, 'checkpoint', 'teacher_resnet56_best.pth')
    teacher_ckpt = Path(r"E:\paper\transformer-master\KD\KD-pytorch-master\checkpoint\Ter_resnet56-best.pth")
    
    print(f"Teacher checkpoint path: {teacher_ckpt}")
    student_ckpt = Path(__file__).resolve().parent / 'checkpoint' / 'student_resnet20_adaptive_layer2.pth'
    ...
    if resume :
        ...
        if os.path.exists(checkpoint_path):
            print(f"检测到本地权重，正在加载: {checkpoint_path}")
            # map_location 是为了防止 GPU/CPU 环境不一致报错
            state_dict = torch.load(checkpoint_path, map_location=device)
            student.load_state_dict(state_dict['net'])
            optimizer.load_state_dict(state_dict['optimizer'])
            scheduler.load_state_dict(state_dict['scheduler'])
            start_epoch = state_dict['epoch'] + 1
            best_acc = state_dict.get('best_acc', best_acc)
    else:
        print("未找到本地权重，将从官方预训练开始全新训练")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_acc > best_acc:
            ...
            torch.save(
                {'net': model_to_save.state_dict(), 'acc': val_acc, 'epoch': epoch,'optimizer': optimizer.state_dict(),'scheduler': scheduler.state_dict(),'best_acc': best_acc},
                student_ckpt
            )
            best_acc = val_acc
        scheduler.step()
