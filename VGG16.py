...
def main():
    ...
    print("==> 初始化 VGG16 教师模型...")
    model = VGG('VGG16').to(device)
    model.classifier = nn.Linear(512, 100).to(device)

    # model = vgg19(weights = VGG19_Weights.IMAGENET1K_V1)
    # # 适配输入大小（32x32）
    # model.features[0] = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)
    # # 可选：移除第一个最大池化层（保留更多细节）
    ...
    save_path = Path(__file__).resolve().parent / "checkpoint/VGG16-best.pth"
    if save_path.exists():
        checkpoint = torch.load(save_path, map_location=device)
        model.load_state_dict(checkpoint["net"])
        best_acc = checkpoint.get("acc", best_acc)
        print(f"模型已加载，最优准确率为 {best_acc:.2f}%")
    # 5. 定义损失函数、优化器和学习率调度器
    ...
    # 最终只评估一次测试集
    ckpt = torch.load('./checkpoint/teacher_resnet50_best.pth', map_location=device)
    model.load_state_dict(ckpt['net'])
    test_loss, test_acc = evaluate(model, testloader, criterion, device)

    print(f"\n✅ 训练完成！最佳验证准确率: {best_acc:.2f}%")
    print(f"Final Test Results: Loss: {test_loss:.4f} | Acc: {test_acc:.2f}%")
    print("权重已保存至: ./checkpoint/teacher_resnet50_best.pth")
