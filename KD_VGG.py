...
def distill_loss(student_logits, teacher_logits, student_feat, teacher_feat, labels, alpha, beta, base_t):
    ...
    with torch.no_grad():
        ...
        max_entropy = torch.log(
            torch.tensor(teacher_logits.size(1), device=teacher_logits.device, dtype=teacher_logits.dtype)
        )
...
def extract_vgg_feature(vgg_model, x, feature_layer='block4'):
    # VGG features are sequential; use block names to align with teacher layer depth.
    block_end_idx = {
        'block1': 6,
        'block2': 13,
        'block3': 26,
    ...
    if feature_layer not in block_end_idx:
        raise ValueError(f'Unsupported feature_layer: {feature_layer}')

    end_idx = block_end_idx[feature_layer]
    feat = x
    ...
    out = vgg_model.features(x)
    out = out.view(out.size(0), -1)
    logits = vgg_model.classifier(out)
    return logits, feat
...
def train(epoch, student, teacher, trainloader, optimizer, device, alpha, beta, base_t, epochs, normal_kd):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        student_logits, student_feat = extract_vgg_feature(student, inputs, feature_layer='block4')
...
def main():
    ...
    teacher_ckpt = Path(r"E:\paper\transformer-master\KD\KD-pytorch-master\checkpoint\Pre_VGG16-best.pth")
    student_ckpt = Path(__file__).resolve().parent / 'checkpoint' / 'student_vgg11_adaptive_layer2.pth'
    ...
    print('==> Building teacher (ResNet50) and student (VGG19)..')
    teacher = VGG('VGG16').to(device)
    student = VGG('VGG11').to(device)
    student.classifier = nn.Linear(512, num_classes).to(device)
    teacher.classifier = nn.Linear(512, num_classes).to(device)
    ...
    for epoch in range(1, epochs + 1):
        train(epoch, student, teacher, trainloader, optimizer, device, alpha, beta, base_t, epochs, normal_kd)
        ...
        if val_acc > best_acc:
            ...
            torch.save(
                ...
                os.path.join('checkpoint', student_ckpt)
            )
            best_acc = val_acc

        scheduler.step()

    best_model_path = os.path.join('checkpoint', student_ckpt)
