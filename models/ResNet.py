...
class ResNet(nn.Module):
    def __init__(self, dataset='cifar100', depth=18, num_classes=10, bottleneck=False):
        ...
        if self.dataset.startswith('cifar'):
            ...
            self.relu = nn.ReLU(inplace=True)
            self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  #进入layer1之前的下采样
            self.layer1 = self._make_layer(blocks[depth], 64, layers[depth][0])
            self.layer2 = self._make_layer(blocks[depth], 128, layers[depth][1], stride=2)
            self.layer3 = self._make_layer(blocks[depth], 256, layers[depth][2], stride=2)
            self.layer4 = self._make_layer(blocks[depth], 512, layers[depth][3], stride=2)
            self.avgpool = nn.AvgPool2d(7)#nn.AdaptiveAvgPool2d((1, 1)) 是PyTorch实现GAP的方式，自动将任意尺寸的 H×W 池化为 1×1。
